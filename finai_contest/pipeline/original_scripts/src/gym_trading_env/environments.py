...
class TradingEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Changed to accept 'finrl_df' (FinRL multi-tic dataframe). If provided, convert it to the internal
        single-instrument OHLCV + features dataframe. If both provided, `df` takes precedence.
        """
        # defaults
        self.positions = positions if positions is not None else [-1, 0, 1]
        ...
        self.reward_function = reward_function if reward_function is not None else basic_reward_function
        ...
        # Prefer the explicit df if given
        if df is None and finrl_df is not None:
            df = self._convert_finrl_df(finrl_df, finrl_tic)
        
        if df is None:
            raise ValueError("Either df or finrl_df must be provided to TradingEnv")
        
        # Ensures index is datetime index (many methods rely on self.df.index)
        if not isinstance(df.index, pd.DatetimeIndex):
            # Tries common datetime columns
            if "date" in df.columns:
                ...
                raise ValueError("DataFrame must have a datetime index or a 'date'/'Time' column")

        # Sets dataframe and feature arrays
        self._set_df(df)
    ...
    @staticmethod
    def _convert_finrl_df(finrl_df: pd.DataFrame, tic: str | None = None) -> pd.DataFrame:
        """
        Convert FinRL-style dataframe (columns: date, tic, open, high, low, close, volume, ...)
        into the single-instrument OHLCV + indicator dataframe expected by the env.
        If `tic` is None and multiple tics present, raise — caller should pick one.
        """
        if "date" not in finrl_df.columns:
            raise ValueError("finrl_df must contain 'date' column")
        if "tic" not in finrl_df.columns:
            # treat as already single-instrument FinRL-style
            single = finrl_df.copy()
        else:
            ticks = finrl_df["tic"].unique()
            if tic is None:
                if len(ticks) > 1:
                    raise ValueError("finrl_df contains multiple tickers; pass finrl_tic to select one")
                tic = ticks[0]
            single = finrl_df[finrl_df["tic"] == tic].copy()

        single = single.sort_values("date")
        ...
        single["Time"] = pd.to_datetime(single["Time"])
        # ensure numeric OHLCV
        for col in ["Open", "High", "Low", "Close", "Volume"]:
            if col in single.columns:
                single[col] = pd.to_numeric(single[col], errors="coerce")
        # keep indicator columns as-is (FinRL uses INDICATORS names)
        cols = ["Time", "Open", "High", "Low", "Close", "Volume"]
        extra = [c for c in single.columns if c not in cols and c != "tic"]
        out = single[cols + extra].dropna().reset_index(drop=True)
        return out
    
    def get_sb_env(self):
        """
        Return SB3-compatible vec env and initial obs so downstream FinRL notebook code works:
